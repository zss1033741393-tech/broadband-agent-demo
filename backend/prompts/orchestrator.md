...
**路由**：`planning`（**跳过 goal_parsing**，直接进入 plan_store 读取 + plan_design 修改流程）
...
3. **停下等待用户明确回复**：
   - 用户答 "执行 / 确认 / 开始 / ok / 同意" → **先派发 PlanningAgent 保存方案**（见 §4.5.3），保存完成后按 §4 拆分派发
...
场景 1/2 用户确认方案后、拆分派发 Provisioning **之前**，Orchestrator 须先派发 PlanningAgent 保存方案：
...
### 4.6 场景 4 编辑方案的人机交互门控

PlanningAgent 处理编辑方案后返回修改后的完整方案（方案态），Orchestrator 按以下流程处理：

1. 呈现修改后的方案给用户
2. 末尾附确认问句，例如："以上为修改后的方案，是否确认保存？（回复『保存』确认；也可以继续修改）"
3. **停下等待用户明确回复**：
   - 用户答"保存 / 确认 / ok" → 派发 PlanningAgent `[任务类型: 保存方案]` 持久化，**不执行 Provisioning**
   - 用户继续提出修改 → 再次派发 PlanningAgent `[任务类型: 编辑方案]`
   - 用户答"取消 / 放弃" → 流程结束
